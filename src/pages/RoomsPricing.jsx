  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="View affordable room pricing at Meridian Homestay Newtown. Options for single bed, double sharing with or without food." />
      </Helmet>
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {plans.map((plan, idx) => (
              ...
              >
                {plan.popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-yellow-400 text-yellow-900 text-sm font-bold rounded-full border-4 border-white shadow-sm flex items-center gap-1">
                    <Sparkles className="w-4 h-4" /> HIGHEST VALUE
                  </div>
                )}
                
                <h3 className="font-heading text-2xl font-bold mb-2 text-center mt-2">{plan.title}</h3>
                <div className="text-center mb-6">
                  <span className="text-4xl font-bold">{plan.price}</span>
                  <div className={`text-sm mt-2 font-medium ${plan.popular ? 'text-blue-200' : 'text-gray-500'}`}>
                    {plan.subtitle}
                  </div>
                </div>
                ...
                <div className="space-y-4 mb-10">
                  {plan.features.map((feature, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <CheckCircle2 className={`w-5 h-5 shrink-0 mt-0.5 ${plan.popular ? 'text-blue-300' : 'text-blue-600'}`} />
      ...
      <section className="py-16 px-4 bg-zinc-50 border-t border-gray-100">
        <div className="container mx-auto max-w-4xl text-center">
          ...
          <h2 className="font-heading text-2xl md:text-3xl font-bold text-gray-900 mb-4">Security Deposit Information</h2>
          <p className="text-gray-600 text-lg">
            A refundable security deposit equal to setup charges is required during admission. Please contact our management at <a href="tel:+918282895500" className="text-blue-600 font-medium">8282895500</a> to get exact specific breakdowns based on your room selection.
          </p>
        </div>
      </section>
